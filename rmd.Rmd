...
Vamos mostrar um exemplo simples de um gráfico de linhas utilizando séries temporais e algumas maneiras de deixar ele mais elaborado. Para isso, vamos reconsiderar os dados utilizados em um post anterior, sobre os casos do novo coronavirus no Estado do Espírito Santo. Como no post já mostramos detalhadamente como importar e utilizar os dados, vamos pular essa etapa. vocês podem ver todas as instruções no seguinte [link](https://daslab-ufes.github.io/covid-es-r/). Uma pequena observação a ser feita, no post deixamos um link para baixar diretamente a base de dados, porém, o link mudou. Caso queira ter acesso a base de dados basta clickar [aqui](https://bi.s3.es.gov.br/covid19/MICRODADOS.csv). Deviso a isso, os posts foram feitos com dados de períodos diferentes, podendo então haver algumas divergências nos gráficos.
...
Uma observação a ser feita. Ocultamos a parte do tratamente e paração da base de dados pois ja produzimos um post mostrando passo a passo como fazer isso.   caso tenha interesse nesse assunto, basta clickar [aqui](https://daslab-ufes.github.io/highcharter/). 
...
ggplot(dados_sumarizados_5,
  ...
  geom_point() + 
  theme(legend.position = "bottom") +
  scale_x_date(limit=c(as.Date("2021-03-22"),as.Date("2021-04-23")),date_breaks = "1 week")
...
Acima fizemos dois gráficos de calor, um simples e o outro um pouco mais elaborado. Gráficos de calor podem ser difíceis de visualizar as vezes. Neste caso, pode ser interessante fazer o uso de gráficos interativos e vamos falar disso agora.
...
o pacote `highcharter` também serve para fazer gráficos interativos. Já fizemos um post muito interessante para mostrar o uso desse pacote e você pode acessar clicando aqui [aqui](https://daslab-ufes.github.io/highcharter/). Segue um spoiler do que vai encontrar no post:
